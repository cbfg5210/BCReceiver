 ...
 * 修改内容：
 */
class NetworkWatcher : BCWatcher {
    ...
    private var netConnAction: ((isConnected: Boolean, isAvailable: Boolean, netType: Int) -> Unit)? =
        null
    ...
    private fun handle(intent: Intent) {
        when (intent.action) {
            ...
            WifiManager.SCAN_RESULTS_AVAILABLE_ACTION -> {
                wifiScanResultAction?.run { this.invoke(wifiManager.scanResults) }
            }
        }
    }
